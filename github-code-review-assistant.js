...
// @author          http://jakub-g.github.com/ - enhanced by Aaron Lademann
// @downloadURL     https://raw.github.com/alademann/misc/master/github-code-review-assistant.js
// @grant                none
...
(function(window, undefined) {
    ...
    if (w.self != w.top && document.location.href.indexOf("/plugins/servlet/gadgets/ifr") === -1){
        return;
    }
    ...
    var CONFIG = {};
    CONFIG.advancedCollapsing = false;
    ...
    CONFIG.footerSize = 8;
    CONFIG.sidebarColor1 = '#d8d8d8';
    CONFIG.sidebarColor1Hover = '#fafafa';
    CONFIG.sidebarBorder = '#ccc';
    CONFIG.sidebarText = '\25b4';
    CONFIG.sidebarTextFooter = '\25b4 Top of Diff \25b4';

    CONFIG.showDiffSidebar = false;
    CONFIG.showDiffFooter = true;
    ...
    gha.util.DomWriter = {};
    gha.util.locationHash = location.hash;
    // the css classes we'll use primarily to affect the visibility of the diffs
    gha.util.hideClass = 'hide';
    gha.util.collapseClass = 'collapse';
    ...
    gha.util.DomWriter.attachGlobalCss = function () {
        ...
        css.push('.' + gha.util.collapseClass + '.meta .show-inline-notes,\
            .' + gha.util.collapseClass + ' .meta .show-inline-notes {\
            display: none !important;\
        }');
        ...
        if (CONFIG.enableDiffSidebarAndFooter) {
            ...
            if(CONFIG.showDiffFooter) {
                css.push('.ghAssistantFileFoot {\
                    height: ' + CONFIG.footerSize + 'px;\
                    border: 1px solid ' + CONFIG.sidebarColor1 +';\
                    border-top-color: ' + CONFIG.sidebarColor1 + ';\
                    border-left-color: ' + CONFIG.sidebarColor1 + ';\
                    background-color: ' + CONFIG.sidebarColor1 + ';\
                    font-size: 1px;\
                    line-height: 1px;\
                    text-align: center;\
                }');
                css.push('.ghAssistantFileFoot a {\
                ...
                css.push('.ghAssistantFileFoot:hover {\
                    background-color: ' + CONFIG.sidebarBorder + ';\
                    border-color: ' + CONFIG.sidebarBorder + ';\
                }');
            }

            if(CONFIG.showDiffSidebar) {
                css.push('.ghAssistantFileSide {\
                    width: '+ CONFIG.sidebarSize + 'px;\
                    border: 1px solid ' + CONFIG.sidebarBorder + ';\
                    border-right-color: ' + CONFIG.sidebarColor1 + ';\
                    background-color: ' + CONFIG.sidebarColor1 + ';\
                    font-size: 1px;\
                    line-height: 1px;\
                    height: 100%;\
                ...
                css.push('.ghAssistantFileSide a {\
                    box-sizing: border-box;\
                    display: block;\
                    width: '+ CONFIG.sidebarSize + 'px;\
                    height: 100%;\
            ...
            // line up our minibuttons with theirs
            css.push('.minibutton.custom {padding-bottom: 1px !important;}');
            css.push('.minibutton.custom.no-group {margin-right: 10px !important;}');
        }
    ...
    gha.util.DomWriter.attachCollapseExpandDiffsButton = function (hiddenByDefault) {
        ...
        newButton.className = 'minibutton custom no-group';
    ...
    gha.util.DomWriter._attachReviewStatusButton = function (diffContainer, text /*also cssClassNamePostfix*/) {
        ...
        var newButton = document.createElement('a');
        newButton.className = 'minibutton custom';
        newButton.href = '#';
    ...
    gha.util.DomWriter._attachSidebarAndFooter = function (child) {
        ...
        var vLink = '<a title="' + L10N.sidebarFooterTooltip + '" href="#' + diffContainer.id + '"> </a>';
    ...
    gha.util.VisibilityManager.hideLongDiffs = function(minDiff) {
        ...
        for(var i=0, ii = nbOfCommits; i<ii; i++) {
            ...
            if(CONFIG.advancedCollapsing) {
                // try to match up any #hash link to a discussion within a diff
                if(hashInUrl.length > 0 && ! foundDiscussion) {
                    // if we havent yet matched up the ID in the url... keep looking in each diff until we find it.
                    var discussions = diffContainer.querySelector('.inline-comment');
                    var discussionId = null;
                    var discussionInUrl = false;

                    if(discussions !== null) {
                        discussionId = discussions.getAttribute('id');
                        try {
                            if(discussionId && hashInUrl.lastIndexOf(discussionId) > -1) {
                                discussionInUrl = true;
                                foundDiscussion = true; // don't go through this loop again
                            } else {
                                discussionInUrl = false;
                            }
                        } catch(err) {
                            console.log("ERROR: " + err);
                            foundDiscussion = true; // don't loop again
                            discussionInUrl = false;
                        }
                    }
                ...
                // try to match up any #hash link to a line number within a diff
                if(hashInUrl.length > 0 && ! foundLineNumber) {
                    // if we havent yet matched up the ID in the url... keep looking in each diff until we find it.
                    var lineNos = diffContainer.querySelector('.diff-line-num');
                    var lineNoId = null;
                    var lineNoInUrl = false;

                    if(lineNos !== null) {
                        lineNoId = lineNos.getAttribute('id');
                        try {
                            if(lineNoId && hashInUrl.lastIndexOf(lineNoId) > -1) {
                                lineNoInUrl = true;
                                foundLineNumber = true; // don't go through this loop again
                            } else {
                                lineNoInUrl = false;
                            }
                        } catch(err) {
                            console.log("ERROR: " + err);
                            foundLineNumber = true; // don't loop again
                            lineNoInUrl = false;
                        }
                    }

                    if (bKeepItemFromUrlHash && lineNoInUrl) {
                        // don't collapse this file - it contains a diff comment
                        continue;
                    }
                }
    ...
    gha.util.VisibilityManager.toggleDisplayAll = function(bVisible, bKeepItemFromUrlHash, fromButton) {
        ...
        var newDisplayClass = bVisible ? 'block' : 'none';
    ...
    gha.util.ClickHandlers.createReviewButtonHandler = function (text, diffContainer) {
        return function(evt) {
            ...
            evt.preventDefault(); // don't change location.hash to #
